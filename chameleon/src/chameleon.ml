...
let output_camlreturn_prefix = function
  | Int | Bool -> cprintf "CAMLreturnT(int, "
  | String -> cprintf "CAMLreturnT(char *, "
  | Abstract -> cprintf "CAMLreturnT(value, "
  | Unit -> assert false
...
let c_to_ml x = function
  ...
  | String -> cprintf "caml_copy_string(%s)" x
...
let output_c_stub f typ mltyp = 
  ...
  cprintf "  r = caml_callback%s_exn(*%s_%s_rv," (arity f mltyp) !base f;
  ...
  if mltyp.result <> Unit then begin
    output_camlreturn_prefix mltyp.result;
  ...
  cprintf "; };\n";
  cprintf 
    "  ics_error(\"%s_%s\",caml_format_exception(Extract_exception(r)));\n" !base f;
...
let output_c_indirect_stub f typ mltyp = 
  ...
  cprintf "  caml_register_global_root(r);\n";
  cprintf "  *r = 1;\n";
  cprintf "  set_ocaml_handlers();\n";
  cprintf "  *r = caml_callback%s_exn(*%s_%s_rv," (arity f mltyp) !base f;
...
let constructors () =
  ...
  cprintf 
    "\nvalue* %s_void()  { value* r = malloc(sizeof(value)); caml_register_global_root(r); *r = Val_int(0); return r; }\n" !base;
  cprintf "\nvalue %s_direct_false() { return Val_int(0); }" !base;
  cprintf 
    "\nvalue* %s_false() { value* r = malloc(sizeof(value)); caml_register_global_root(r); *r = Val_int(0); return r; }\n" !base;
  cprintf "\nvalue %s_direct_true()  { return Val_int(1); }" !base;
  cprintf 
    "\nvalue* %s_true()  { value* r = malloc(sizeof(value)); caml_register_global_root(r); *r = Val_int(1); return r; }\n" !base;
  ...
  cprintf "  if (((*r) & 1) == 0) { caml_remove_global_root(r); }\n}\n";
...
let output_code l =
  ...
  cprintf "#include <caml/printexc.h>\n";
  ...
  lprintf 
    ...
    !base;
  List.iter 
    (fun (rname,_,_,_) -> cprintf "const value * %s_%s_rv;\n" !base rname)
    l;
  constructors();
  List.iter 
  ...
  cprintf " a[0] = malloc(4 * sizeof(char));\n";
  cprintf " a[0][0] = 'i';\n";
  cprintf " a[0][1] = 'c';\n";
  cprintf " a[0][2] = 's';\n";
  cprintf " a[0][3] = '\\0';\n";
