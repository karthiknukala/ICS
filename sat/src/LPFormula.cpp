...
static std::hash<const char *> proc;
...
void LPFormulaManager::dump_formula(std::ostream & target, const LPFormula * f, bool negated) const
{
	if (negated)
		target << "(~ ";
	switch(f->get_kind()) {
...
{
  std::cout << "[memory] Formula manager memory info: " << std::endl;
  std::cout << "  formula array size: " << num_formulas << std::endl;
  std::cout << "  number of formulas: " << (next_formula_id - 1) << std::endl;
  std::cout << "  argument array size: " << num_arguments << std::endl;
  std::cout << "  number of arguments: " << (next_argument_pos - 1) << std::endl;
  std::cout << "  pool size: " << formula_pool.size() << std::endl;
	// cout << "  pool max size: " << formula_pool.max_size() << endl;
  std::cout << "  memory consumed: " << ((sizeof(LPFormula) * num_formulas +
...
std::ostream& operator<<(std::ostream& target, const LPFormulaManager & m)
{
