...
typedef std::unordered_set<LPFormulaId, LPFormulaIdHashFcn, LPFormulaIdEqualTo> LPFormulaPool; 
...
class LPFormulaManager {
  ...
  friend std::ostream& operator<<(std::ostream& target, const LPFormulaManager & m);
...
public:
  ...
  void dump_formula(std::ostream & target, const LPFormula * f, bool negated = false) const ;
  void dump_formula(std::ostream & target, LPFormulaId f) const { dump_formula(target, get_formula(f), f < 0); }
