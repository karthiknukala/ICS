...
std::unordered_set<const char *, std::hash<const char *>, eqstr> gUStringSet; 
#ifndef USTRING_BLOCK_SIZE
#define USTRING_BLOCK_SIZE 8192
#endif

std::list<char *> gUStringMemoryBlocks;
...
{
  std::unordered_set<const char *, std::hash<const char *>, eqstr>::const_iterator it = gUStringSet.find(s);
...
void UString::resetMemory()
{
  ...
  std::list<char *>::iterator it = gUStringMemoryBlocks.begin();
...
std::hash<const char *> hashUString::proc;
