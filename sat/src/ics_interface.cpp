...
// 					}
	...
	DBG_CODE(FormulaIdMapping::const_iterator it = associated_formulas.begin();
					 for (;it != associated_formulas.end(); it++) {
						 ...
						 std::cout << "Associated formulas  of " << curr_idx << ":\n";
...
bool ICSInterface::assert_formula(LPFormulaId f, bool in_scratch) {
	...
	DBG_CODE(
		 std::cout << "asserting "; std::cout.flush();
					 icsat_atom_pp(f_v);
		 std::cout << ".\n";
					 );

 	DBG_CODE(std::cout << std::endl << "[ics] asserting formula " << f << ", ID = " << f_v << std::endl;);
	...
	ics_elapsed += ((double) (end - start)) / CLOCKS_PER_SEC;
 	DBG_CODE(std::cout << "   result is: " << result << std::endl;);
	DBG_CODE(
					 if (result == 0) {
					   std::cout << "ICS DETECTED INCONSISTENCY:\n";
					   std::cout << "  ASSERTING:\n   ";
						 icsat_atom_pp(f_v);
						 std::cout << "\n\n  STACK:\n";
						 icsat_stackpp(); std::cout << std::endl;
						 std::cout << "------------------------------\n\n";
					 }
					 );
	return result == 1;
...
void ICSInterface::dump_current_state() 
{
  ...
  std::cout << "TO DO (dump_current_state): print the ICS current state\n";
}

void ICSInterface::dump_ics_formula(LPFormulaId f_id)
{
  std::cout << "TO DO (dump_ics_formula): print the ICS atom\n";
}
...
std::pair<int *,int> ICSInterface::explain()
{
  ...
  return std::pair<int*,int>(result, len);
}
