...
LPSolver::LPSolver(LPFormulaManager * f_manager, bool use_proof_objects,
		   unsigned int initial_capacity, unsigned int initial_number_of_clauses,
...
void LPSolver::dump_internal_info(std::ostream & target)
{
...
void LPSolver::dump_to_check_vector(std::ostream & target) {
...
bool LPSolver::is_satisfiable(LPFormulaId f)
{
	...
	if (verbose)
		std::cerr << "    internalizing formula...\n";
	...
	if (verbose)
		std::cerr << "    propagating extended constraints...\n";
	...
	if (verbose)
		std::cerr << "    preparing depth first search...\n";
	...
	if (verbose)
		std::cerr << "  branching (depth first search)...\n  ";
...
void LPSolver::dump_atomic_formula_values(std::ostream & target, LPFormulaId f)
{
	queue<unsigned int> & to_dump = tmp_queue;
	to_dump.reset();
...
void LPSolver::dump_clauses(std::ostream & target)
{
...
void LPSolver::dump_clause(std::ostream & target, LPClause & clause)
{
	unsigned int n = clause.get_size();
	for (unsigned int i = 0; i < n; i++) {
