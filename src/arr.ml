...
module T: Can.T = struct
  ...
  and of_var_equal3 e (p, s) =
    let (u0, rho) = P.find p (Fact.Equal.rhs_of e) in
      S.Dep.fold s
	(fun e1 acc1 ->
	   try
		 ...
		 P.fold p
		   (fun e2 acc2 ->
			...
			let (u', (a, i, x), sigma) = d_update p e2 in
			  match P.is_equal p u' u0, P.is_diseq p i j with
			    | Some(theta), Some(ups) -> 
  ...
  and of_var_equal4 e (p, s) =
    let (v0, rho) = P.find p (Fact.Equal.rhs_of e) in
      S.Dep.fold s
	(fun e1 acc1 ->
	   try
	   ...
	   let (u', (v, j), rho) = d_select p e1 in
	     S.iter
		  ...
		  let (v', (a, i, x), tau) = d_update p e2 in
		    match P.is_equal p v v' with
		      | None -> ()
		      | Some(sigma) ->
			  match P.is_equal_or_diseq p i j with
			    | Jst.Three.X -> 
				let theta = Jst.dep3 rho tau sigma in
				let cl = Clause.of_list ([Atom.mk_equal (i, j);
							 Atom.mk_diseq (i, j)], theta)
				in
				  raise(Found(cl))
			    | _ -> ())
	       s)
